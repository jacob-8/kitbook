...
This function will take your current version of playwright and your Kitbook config ([[0-config|read here for quick setup]]) and construct urls to each variant found in your `variants.ts` files. It snapshots each url for all desired viewports (and [[8-i18n|languages]] if applicable). A GitHub action is being built to then enable changed snapshots to be commented directly onto a PR.
