...
export function compressToEncodedURIComponent(input: string) {
  ...
  return _compress(input, 6, function (index: number) { return keyStrUriSafe.charAt(index); });
}
...
export function decompressFromEncodedURIComponent(input: string) {
  ...
  return _decompress(input.length, 32, function (index: number) { return getBaseValue(keyStrUriSafe, input.charAt(index)); });
}
